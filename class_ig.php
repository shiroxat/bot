...
function findProfile($username, $session = 0)
{
    ...
    if (strpos($get, 'The link you followed may be broken, or the page may have been removed.')) {
        ...
        if (empty($result->edge_owner_to_timeline_media->edges) && $result->edge_owner_to_timeline_media->count >= 1) {
            ...
            $follow_you = ($result->follows_viewer) ? 'true' : 'false' ;
...
function login($username, $password)
{
    ...
    if (strpos($post, '{"authenticated": false')) {
        ...
        $url .= getStr($post, '"checkpoint_url": "', '", "lock":');
        $data = array(

            'action' => 'login',
            'status' => 'error',
            'username' => $username, 
            'details' => 'checkpoint required',
            'url' => $url,
        );
...
function comment($id, $session, $text)
{
    ...
    }elseif (strpos($post, 'Unable to post comment.')) {

        $data = array(
            'action' => 'comment',
            'status' => 'fail',
            'details' => 'Unable to post comment.',
        );
...
function follow($username, $session)
{
    ...
    if (isset($profile['id'])) {
        ...
        if ($profile['is_follow'] == 'true') {
            ...
            }elseif (strpos($post, 'It looks like you were misusing this feature by going too fast. You’ve been temporarily blocked from using it.')){
                
                $data = array(
                    'username' => $username,
                    'action' => 'follow',
                    'status' => 'error',
                    'details' => 'temporarily blocked',
                );
    ...
    if (strpos($get, 'The link you followed may be broken, or the page may have been removed.')) {
        ...
        if (empty($result) && $data_array->entry_data->ProfilePage['0']->graphql->user->edge_owner_to_timeline_media->count >= 1) {
            ...
            $data = array(
                ...
                // 'captions' => $result->edge_media_to_caption->edges['0']->node->text,
